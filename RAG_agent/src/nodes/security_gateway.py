...
Architecture:
    User Input → Security Gateway → Preprocessing → Intent Classification → RAG → Agent
"""
...
def security_gateway_node(state: AgentState) -> Dict[str, Any]:
    ...
    each piece of the input independently, as per requirements.
    ...
    Returns:
        Updated state with:
            ...
            - final_response (str): Error message if blocked
            - continue_conversation (bool): False if blocked

    Raises:
        ValueError: If user_input is missing from state
    """
    user_input = state.get("user_input")

    if not user_input:
        raise ValueError("user_input is required for security gateway validation")
    ...
    logger.info(f"Security gateway validating input: {user_input[:100]}...")
    ...
    try:
        ...
        validate_user_input(text=user_input, raise_on_malicious=True)
        ...
        # Block the request by setting final_response and ending conversation
        return {
